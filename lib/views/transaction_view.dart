          ...
          children: [
            FutureBuilder(
              ...
              builder: (context, snapshot) {
                if (snapshot.hasData) {
                  var resultado = snapshot.data! as Map<String, dynamic>;
                  if (resultado["ok"] == false) {
                    return Container();
                  }
                  var transacoes =
                      resultado["transactions"] as List<Map<String, dynamic>>;
                  var lista_acoes =
                      transacoes.map((e) => e["ticker"]).toSet().toList();
                  var lista_carteiras =
                      transacoes.map((e) => e["wallet_name"]).toSet().toList();
                  List<DropdownMenuEntry> type_options = [
                    DropdownMenuEntry(value: "", label: ""),
                    DropdownMenuEntry(value: "VENDA", label: "Vendas"),
                    DropdownMenuEntry(value: "COMPRA", label: "Compras"),
                  ];
                  ...
                  lista_acoes.forEach((element) {
                    stocks_options
                        .add(DropdownMenuEntry(label: element, value: element));
                  });
